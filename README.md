...
- **明细汇总**：从零件名解析**图号、材料**，从装配体统计**加工件数量**，可导出 CSV、可把结果写回零件自定义属性。
...
1. 数据源选「当前文档」：打开装配体 → 得到加工件清单；打开零件 → 得到该零件的图号/材料。
2. 或选「文件夹」，批量汇总一批零件。
3. 点「开始汇总」→ 结果表 → 可「导出 CSV」或「写回零件属性」。

输出列：`序号 | 图号 | 名称 | 材料 | 数量 | 类型 | 配置 | 文件名`
...
「标准件（前缀开头）」设置页只负责维护前缀：输入名称后点「增加」，或点击当前前缀上的 `×` 删除。
