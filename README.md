...
- **BOM 预览/明细汇总**：窗口打开后自动汇总，可选择总装、子装配体或单个零件作为 BOM 范围，直接编辑**名称、材料、工艺**并写回零件自定义属性，也可导出 CSV。
...
加工件规则支持分隔符 `_` 或 `-`，并按界面中从上到下的段顺序解析：
...
- 默认顺序为：**日期、材料/工艺、零件名称、变更序号、拓展代号**
- 变更序号、拓展代号及其他**自定义段**可通过 `＋ 增加段` 添加
- 新增的**自定义段**可直接输入名称，例如“供应商”“表面处理”“项目号”，保存后会保留
- 除时间段外，可用 `↑ / ↓` 调整顺序，也可删除；材料和名称会按调整后的段位置解析
- 勾选某段右侧的“并入 BOM 名称”，该段会和零件名称一起显示在 BOM；例如勾选第 3～5 段后，`板1_A_左装` 会作为 BOM 零件名
